import { Injectable } from '@angular/core';
import { HttpClient } from '@angular/common/http';
import { Observable } from 'rxjs';
import { BASE_API_URL } from '../../../../invoicing-ui/src/app/Constants';

@Injectable({
  providedIn: 'root'
})
export class ProductService {
  //private baseUrl = 'https://localhost:7025/api/Product';
  private baseUrl = `${BASE_API_URL}/Product`;
  constructor(private http: HttpClient) {}

  getProducts(companyId: string): Observable<any[]> {
    return this.http.get<any[]>(`${this.baseUrl}?companyId=${companyId}`);
  }

  getProduct(id: string): Observable<any> {
    return this.http.get<any>(`${this.baseUrl}/${id}`);
  }

  createProduct(model: any): Observable<any> {
    return this.http.post<any>(this.baseUrl, model);
  }

  updateProduct(id: string, model: any): Observable<any> {
    return this.http.put<any>(`${this.baseUrl}/${id}`, model);
  }

  deleteProduct(id: string): Observable<any> {
    return this.http.delete<any>(`${this.baseUrl}/${id}`);
  }

  // ---- BOM endpoints ----

  getBom(productId: string, companyId: string): Observable<any[]> {
    return this.http.get<any[]>(
      `${this.baseUrl}/${productId}/bom?companyId=${companyId}`
    );
  }

  saveBom(productId: string, companyId: string, lines: any[]): Observable<any> {
    return this.http.post<any>(
      `${this.baseUrl}/${productId}/bom?companyId=${companyId}`,
      lines
    );
  }
}
